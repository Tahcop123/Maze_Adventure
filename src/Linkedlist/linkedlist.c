...
linkedlistADT NewLinkedList(void)
{
    ...
    head = (linkedlistADT)malloc(sizeof(struct linkedlistCDT));
    head->dataptr = NULL;
	head->next = NULL;
    return (head);
}
...
void FreeLinkedList(linkedlistADT head)
{
	...
	while (nodeptr != NULL) {
		...
		if (nodeptr != head) free(nodeptr->dataptr);
		free(nodeptr);
		nodeptr = nextnodeptr;
	}
}
...
void InsertNode(linkedlistADT head, linkedlistADT nodeptr, void *obj)
{
	...
	ptr = (linkedlistADT)malloc(sizeof(struct linkedlistCDT)); /*New a node*/
